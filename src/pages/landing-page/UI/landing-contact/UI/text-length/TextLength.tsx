...
export const TextLength = ({ watch, field }: Props) => {
	...
	return (
		<p
			className={cn(
				'text-[var(--white)] text-sm font-bold transition-all duration-300 ease-out flex items-center',
				'ml-2.5',
				precent > watchPrecent ? 'h-4 opacity-100 mt-2' : 'h-0 opacity-0 mt-0'
			)}
		>
			{valueLength} <FaGripLinesVertical /> <span className="text-[var(--hot-orange)]">{max}</span>
		</p>
	);
};
